...
module kg.GridSortingTest {
	...
	QUnit.testStart(function () {
		var viewHandle = new go.ActiveManagement.ViewHandle(
			[],
			ko.observableArray([
				new SortableObject("goo", 171, false, new Date(2010, 0, 15)),
				new SortableObject("foo", 7, true, new Date(2012, 0, 14)),
				new SortableObject("bar", 8, false, new Date(2010, 1, 14)),
				new SortableObject("Fooa", 25, true, new Date(2011, 0, 14)),
				new SortableObject("Bara", 41, false, new Date(2010, 0, 14))
			]),
			ko.observable(1),
			function () { }
		);
	...
	test("basic string sorting", function () {
	...
	test("basic number sorting", function () {
		equal(aGrid.items().length, 5, "grid should have 5 items");
	...
	test("basic bool sorting", function () {
		equal(aGrid.items().length, 5, "grid should have 5 items");
	...
	test("basic date sorting", function () {
		equal(aGrid.items().length, 5, "grid should have 5 items");
